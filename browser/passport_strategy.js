...
function OpenIDConnectStrategy(
  ...
  this._key = sessionKey || `oidc:${new URL(this._issuer.issuer).hostname}`;
  ...
  this.name = new URL(client.issuer.issuer).hostname;
}
...
OpenIDConnectStrategy.prototype.authenticate = function authenticate(req, options) {
  (async () => {
    ...
    if (Object.keys(reqParams).length === 0) {
      ...
      if (this._usePKCE && params.response_type.includes('code')) {
        ...
        switch (this._usePKCE) {
          case 'S256':
            params.code_challenge = await codeChallenge(verifier);
    ...
    if (Object.keys(session || {}).length === 0) {
      throw new Error(
        `did not find expected authorization request details in session, req.session["${sessionKey}"] is ${JSON.stringify(
          session,
          null,
          2,
        )}`,
