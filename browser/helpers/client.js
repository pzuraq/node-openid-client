...
const bufferFrom = require('./buffer_from');
const { encode: base64Encode } = require('./base64');
...
async function clientAssertion(endpoint, payload) {
  ...
  if (this[`${endpoint}_endpoint_auth_method`] === 'client_secret_jwt') {
    ...
    return new jose.CompactSign(bufferFrom(JSON.stringify(payload)))
      .setProtectedHeader({ alg })
      .sign(await this.secretForAlg(alg));
  }
...
async function authFor(endpoint, { clientAssertionPayload } = {}) {
  ...
  switch (authMethod) {
    ...
    case 'client_secret_basic': {
      ...
      const value = base64Encode(encoded);
