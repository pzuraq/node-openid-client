...
class Issuer {
  ...
  static async webfinger(input) {
    ...
    if (typeof location.href !== 'string' || !location.href.startsWith('https://')) {
      throw new RPError({
        message: `invalid issuer location ${location.href}`,
        body,
      });
    }
    ...
    if (issuer.issuer !== expectedIssuer) {
      ...
      throw new RPError(
        `discovered issuer mismatch, expected ${expectedIssuer}, got: ${issuer.issuer}`,
      );
    }
    return issuer;
  }
  ...
  static async discover(uri) {
    const wellKnownUri = new URL(uri);
    ...
    if (parsed.pathname.endsWith('/')) {
      wellKnownUri.pathname = `${parsed.pathname}.well-known/openid-configuration`;
    } else {
      wellKnownUri.pathname = `${parsed.pathname}/.well-known/openid-configuration`;
    }
