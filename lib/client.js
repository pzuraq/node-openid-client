...
const tokenHash = require('./helpers/oidc-token-hash');
...
function pickCb(input) {
  return pick(input, ...pickCbFields);
}

function pickCbSearchParams(searchParams) {
  let obj = {};

  for (let field of pickCbFields) {
    if (searchParams.has(field)) {
      obj[field] = searchParams.get(field);
    }
  }
...
class BaseClient {
  ...
  authorizationUrl(params = {}) {
    ...
    const authParams = authorizationParams.call(this, params);

    for (let [key, value] of Object.entries(authParams)) {
...
<form method="post" action="${this.issuer.authorization_endpoint}">
  ...
  endSessionUrl(params = {}) {
    ...
    const target = new URL(this.issuer.end_session_endpoint);
  ...
  callbackParams(input) {
    // TODO: Figure out how to handle this without breaking changes
    // const isIncomingMessage =
    //  input instanceof stdhttp.IncomingMessage || (input && input.method && input.url);
    const isIncomingMessage = input && input.method && input.url;
    ...
    if (isIncomingMessage) {
      switch (input.method) {
        case 'GET':
          return pickCbSearchParams(new URL(input.url).searchParams);
        case 'POST':
          ...
          switch (typeof input.body) {
            ...
            case 'string':
              ...
              //   return pickCbSearchParams(new URLSearchParams(input.body.toString('utf-8')));
              // }
              if (typeof input.body === 'string') {
                return pickCbSearchParams(new URLSearchParams(input.body));
              }
      ...
      return pickCbSearchParams(new URL(input).searchParams);
    }
  }
  ...
  async callback(
    ...
    if (checks.state !== params.state) {
      throw new RPError({
        message: `state mismatch, expected ${checks.state}, got: ${params.state}`,
        checks,
        params,
      });
    }
    ...
    if ('iss' in params) {
      ...
      if (params.iss !== this.issuer.issuer) {
        throw new RPError({
          message: `iss mismatch, expected ${this.issuer.issuer}, got: ${params.iss}`,
          params,
        });
      }
    } else if (
  ...
  async decryptJWE(jwe, expectedAlg, expectedEnc = 'A128CBC-HS256') {
    const header = base64url.decodeJSON(jwe.split('.')[0]);

    if (header.alg !== expectedAlg) {
      throw new RPError({
        message: `unexpected JWE alg received, expected ${expectedAlg}, got: ${header.alg}`,
        jwt: jwe,
      });
    }
    ...
    if (header.enc !== expectedEnc) {
      throw new RPError({
        message: `unexpected JWE enc received, expected ${expectedEnc}, got: ${header.enc}`,
        jwt: jwe,
      });
    }
    ...
    if (expectedAlg.match(/^(?:RSA|ECDH)/)) {
      ...
      plaintext = await jose
        .compactDecrypt(
          jwe,
          await this.secretForAlg(expectedAlg === 'dir' ? expectedEnc : expectedAlg),
        )
        .then(getPlaintext, () => {});
    }
  ...
  async validateIdToken(tokenSet, nonce, returnedBy, maxAge, state) {
    ...
    if (maxAge && payload.auth_time + maxAge < timestamp - this[CLOCK_TOLERANCE]) {
      throw new RPError({
        message: `too much time has elapsed since the last End-User authentication, max_age ${maxAge}, auth_time: ${
          payload.auth_time
        }, now ${timestamp - this[CLOCK_TOLERANCE]}`,
        now: timestamp,
    ...
    if (nonce !== null && (payload.nonce || nonce !== undefined) && payload.nonce !== nonce) {
      throw new RPError({
        message: `nonce mismatch, expected ${nonce}, got: ${payload.nonce}`,
        jwt: idToken,
      });
    }
    ...
    if (this.fapi() && payload.iat < timestamp - 3600) {
      throw new RPError({
        message: `JWT issued too far in the past, now ${timestamp}, iat ${payload.iat}`,
        now: timestamp,
        tolerance: this[CLOCK_TOLERANCE],
        iat: payload.iat,
        jwt: idToken,
  ...
  async validateJWT(jwt, expectedAlg, required = ['iss', 'sub', 'aud', 'exp', 'iat']) {
    ...
    try {
      ...
      throw new RPError({
        message: `failed to decode JWT (${err.name}: ${err.message})`,
        jwt,
      });
    }
    ...
    if (header.alg !== expectedAlg) {
      throw new RPError({
        message: `unexpected JWT alg received, expected ${expectedAlg}, got: ${header.alg}`,
        jwt,
      });
    }
    ...
    if (payload.iss !== undefined) {
      ...
      if (payload.iss !== expectedIss) {
        throw new RPError({
          message: `unexpected iss value, expected ${expectedIss}, got: ${payload.iss}`,
          jwt,
        });
      }
    }
    ...
    if (payload.nbf !== undefined) {
      ...
      if (payload.nbf > timestamp + this[CLOCK_TOLERANCE]) {
        throw new RPError({
          message: `JWT not active yet, now ${timestamp + this[CLOCK_TOLERANCE]}, nbf ${
            payload.nbf
          }`,
          now: timestamp,
          tolerance: this[CLOCK_TOLERANCE],
    ...
    if (payload.exp !== undefined) {
      ...
      if (timestamp - this[CLOCK_TOLERANCE] >= payload.exp) {
        throw new RPError({
          message: `JWT expired, now ${timestamp - this[CLOCK_TOLERANCE]}, exp ${payload.exp}`,
          now: timestamp,
          tolerance: this[CLOCK_TOLERANCE],
          exp: payload.exp,
          jwt,
    ...
    if (payload.aud !== undefined) {
      if (Array.isArray(payload.aud)) {
        ...
        if (!payload.aud.includes(this.client_id)) {
          throw new RPError({
            message: `aud is missing the client_id, expected ${
              this.client_id
            } to be included in ${JSON.stringify(payload.aud, null, 2)}`,
            jwt,
          });
        }
      } else if (payload.aud !== this.client_id) {
        throw new RPError({
          message: `aud mismatch, expected ${this.client_id}, got: ${payload.aud}`,
          jwt,
        });
      }
    }
    ...
    if (payload.azp !== undefined) {
      ...
      if (!additionalAuthorizedParties.includes(payload.azp)) {
        throw new RPError({
          message: `azp mismatch, got: ${payload.azp}`,
          jwt,
        });
      }
    }
    ...
    if (isSelfIssued) {
      try {
        if (!isPlainObject(payload.sub_jwk)) {
          throw new Error('expected sub_jwk to be a plain object');
        }
        const key = await jose.importJWK(payload.sub_jwk, header.alg);
        if (key.type !== 'public') {
          throw new Error('expected key type to be public');
        }
        keys = [{ keyObject: key }];
      } catch (err) {
        throw new RPError({
      ...
      keys = [await this.secretForAlg(header.alg)];
    ...
    if (tokenset.id_token) {
      ...
      if (refreshToken instanceof TokenSet && refreshToken.id_token) {
        ...
        if (actualSub !== expectedSub) {
          throw new RPError({
            message: `sub mismatch, expected ${expectedSub}, got: ${actualSub}`,
  ...
  async userinfo(accessToken, { method = 'GET', via = 'header', tokenType, params, DPoP } = {}) {
    ...
    if (jwt) {
      if (!/^application\/jwt/.test(response.headers.get('content-type'))) {
        throw new RPError({
      ...
      try {
        parsed = JSON.parse(response.parsedBody);
      } catch (err) {
    ...
    if (accessToken instanceof TokenSet && accessToken.id_token) {
      ...
      if (parsed.sub !== expectedSub) {
        throw new RPError({
          message: `userinfo sub mismatch, expected ${expectedSub}, got: ${parsed.sub}`,
          body: parsed,
  ...
  async encryptionSecret(len) {
    const hashAlg = len <= 256 ? 'sha256' : len <= 384 ? 'sha384' : len <= 512 ? 'sha512' : false;
    if (!hashAlg) {
      throw new Error('unsupported symmetric encryption key derivation');
    }

    return (await hash(hashAlg, this.client_secret)).slice(0, len / 8);
  }

  async secretForAlg(alg) {
    if (!this.client_secret) {
      throw new TypeError('client_secret is required');
    }

    if (/^A(\d{3})(?:GCM)?KW$/.test(alg)) {
      return await this.encryptionSecret(parseInt(RegExp.$1, 10));
    }

    if (/^A(\d{3})(?:GCM|CBC-HS(\d{3}))$/.test(alg)) {
      return await this.encryptionSecret(parseInt(RegExp.$2 || RegExp.$1, 10));
    }
  ...
  async requestObject(
    ...
    if (signingAlgorithm === 'none') {
      ...
      if (symmetric) {
        key = await this.secretForAlg(signingAlgorithm);
      } else {
        const keystore = await keystores.get(this);
    ...
    if (fields.alg.match(/^(RSA|ECDH)/)) {
      ...
      key = await this.secretForAlg(fields.alg === 'dir' ? fields.enc : fields.alg);
    }
...
async function dpopProof(payload, privateKeyInput, accessToken) {
  ...
  return new jose.SignJWT({
    ath: accessToken ? base64url.encode(await hash('sha256', accessToken)) : undefined,
    ...payload,
  })
    .setProtectedHeader({
      alg,
